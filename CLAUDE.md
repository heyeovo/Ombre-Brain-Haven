...
GET|PATCH|POST /gateway/api/conversation/agent-wake
       # 读取/CAS 修改窗口 lane 的 wake 控制面；POST accept_user 原子取消尚未触发的 silence timer
...
`recall_policy.py` 的 `RecallNecessityPlan` 在候选相关性之前判断 `none / explicit / contextual`，并用 `targetable` 防止无目标明确请求扩大检索。`gateway.py` 默认以 `OMBRE_RECALL_DECISION_MODE=rebuilt` 让统一 relevance 与代码版 `promote / neutral / reject` utility 接管正式桶结果，最多一张卡；`legacy` 为紧急回滚。rebuilt 审核本轮全部已检索候选，包含旧 admission 已准入但旧排序未选中的桶；其选择排序使用 `score_without_freshness`，保留重要度、相关性和 session 防重复但不使用长期记忆 freshness。Planner degraded 时 contextual 仍不扩张候选，普通 keyword-only 不能靠分数下限证明相关。语义查询默认超时为 5 秒；`explicit` 在 `query_timeout/query_failed/query_embedding_unavailable/query_embedding_failed` 时，可凭“可信主题直接命中桶标题 + keyword >= 0.65”保守进入 Utility，但 `disabled_for_request`、contextual 和仅正文命中不能使用该降级。Debug 的 `formal_bucket_ids` / `legacy_bucket_ids` 保留旧路径，`shadow_bucket_ids` 保留重构投影，`effective_bucket_ids` 记录实际结果，候选记录 `candidate_origin / legacy_score / rebuilt_score / rebuilt_freshness_ignored`，通过 Utility 但未获得单卡位的候选记录在 `eligible_unselected_candidates`，接管时 `affects_recall=true`。旧 source-record 后置追加不得绕过 Utility 或单卡上限。Dashboard session 的 `injected_buckets ∪ session_created_buckets` 排除集合会先从 Hook 候选池移除，并在输出前再次硬过滤；排除桶不能返回，也不能抢占单卡位。Hook 卡片正文后只可附一行最多两个显式关系边的桶名与 ID，不附关联正文。
...
- 正常用户 turn 成功写入时在同一事务按窗口配置只采样一次 `conversation_silence_check_at`，并绑定 `silence_source_turn_id` / `silence_policy_version`；幂等重放不重采样。下一条用户消息进入模型前通过独立原子操作取消仍在未来的 timer。Wake turn 不更新 `last_user_activity_at`，只更新 heartbeat/model 活动；cache deadline 仅随调用端已确认 cache read/write 的请求开始时间刷新。
