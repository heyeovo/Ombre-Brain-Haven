...
GET|POST|PATCH /api/conversation-slices
       # Cookie 认证的切片检查、永久消息原文、额度估算、任务创建/运行/暂停/恢复/重试、人工评价与重切
```
