...
- `rolling_context_json` 保存固定窗口/按天滚动策略、时区、日界小时和逐日 `raw/review/omit` 三态；每次真实修改递增 `context_revision`，记录当时 `context_turn_watermark`，并以 `previous_strategy` 标明紧邻上一 revision 的策略。CC lane 写入同轮 revision，调用端不得用旧 revision 的 `cc_session_id` 直接恢复新配置；Dashboard 用 `previous_strategy=daily_rolling` 对旧持久 transcript 执行强制保真迁移，缺失时不得静默退化成 Haven 正文。滚动模式的桶排除集合只覆盖当前 `raw` 日期内的召回/新建桶，日期变为 `review/omit` 后允许再次召回。
