...
- 日期查询优先看 bucket 的事件时间 `event_time`，按本地年月日匹配；没有 `event_time` 时兼容旧 `date`，两者都没有的旧桶才回退看创建/更新/最后活跃时间。带事件时间的桶不会因为创建日期误入别的日期
...
- 普通聊天窗口不能通过 `hold`、`comment_bucket` 或 `trace` 创建、追加、修改或删除 journey；发现可能的阶段变化时，只在对话中提出候选，由后台周任务统一判断
