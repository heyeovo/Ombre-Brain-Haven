...
`grow` 用于保存值得长期保留的记忆。`hold` 适合短暂抓住当前片段，`comment_bucket` 用于给已有记忆增加年轮。`hold` 成功新建普通 bucket 时，返回文本末尾会附带稳定的 `[bucket_id=<12 位 ID>]` 标记，供会话客户端把本窗口新建 bucket 写入召回排除账本；原有中文结果正文保持兼容。
