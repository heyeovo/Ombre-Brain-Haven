...
事件时间统一以 `metadata.event_time` 为事实源，通常保存完整 ISO 时间；`breath(date="YYYY-MM-DD")` 会按其北京时间年月日匹配。仅当桶没有 `event_time` 时才兼容旧 `metadata.date`，两者都没有的旧桶才回退到创建、更新或最后活跃时间，因此事件桶不会因实际建桶日期被错误带出。
...
正式日回顾保持独立生成：04:30 定时任务和 Dashboard 手动生成都直接调用 `DailyReviewEngine`，不依赖聊天切片 JSON。聊天切片保存在独立 SQLite 表组，不进入 bucket；当前仍是未完成、仅供人工检查的工作流，可在 Dashboard 按日期检查永久 message ID 原文、批准/拒绝、重切或显式运行任务。历史补生成在创建任务前必须展示消息数、预计输入 token 和调用次数，首次最多 14 个真实聊天日；后台暂不自动消费 `raw_exit` / `slice_recovery` 队列。切片目前没有关键词/向量召回，也不会注入正式聊天 Context。
...
`/cc` 私有附件保存在 `buckets_dir/cc-attachments`，SQLite 的 `conversation_attachments` 保存窗口/轮次归属、类型、文件名、MIME、大小、SHA-256，以及文档的受限解析正文。图片接受压缩后的 JPEG/PNG/WebP（单张 ≤2MB），上传时保存文件头中的宽高和按 Anthropic 视觉缩放规则得到的 token 估算；文件接受 PDF/DOCX/MD/TXT/CSV（单个 ≤4MB，为 Dashboard/Vercel multipart 留出请求体余量），并保存实际注入包装的正文 token 估算。解析正文只用于该窗口模型上下文，不进入长期记忆或召回。读取必须通过 Gateway Bearer 鉴权，不生成永久公开 URL；严格写入在同一事务中把有序附件绑定到轮次并纳入幂等指纹。图片与文件可分别单个或按窗口分类清除：删除原文件，文件还会擦除解析正文，但历史保留对应占位元数据和已计算 token；永久删除窗口会同时删除该窗口的附件记录和文件。Dashboard 的“窗口减负”另行复制并瘦身 Claude 本地 transcript，Haven 只原子保存新 lane 指针、保护规则与释放记录，稳定的 Dashboard session 和 `conversation_turns` 原文不会复制。
