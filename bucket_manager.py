...
class BucketManager:
    ...
    def __init__(self, config: dict, embedding_engine=None):
        ...
        self.config = config  # kept for identity_names() access in add_comment()
        ...
        # --- Lexical stop terms & profile cache (Haven) ---
    ...
    async def update(self, bucket_id: str, **kwargs) -> bool:
        ...
        # --- Edit operations don't auto-refresh activation time ---
        # --- 编辑操作不自动刷新激活时间，避免人为维护行为影响浮现权重 ---
    ...
    # --- Bucket ID direct lookup / 桶ID直接读取，跳过语义搜索 ---
        ...
        for bucket in candidates:
            ...
            try:
                ...
                # Also compute per-field fuzzy match for visualization
                match = self._calc_topic_match(query, bucket) if query else {"score": 0.0, "field_scores": {}, "matched_in": []}
                ...
                if self.precise_match_mode and query:
                    ...
                    # --- 4D weighted scoring normalization ---
                    ...
                    # Threshold check for normal path
                    passed_threshold = normalized >= self.fuzzy_threshold

                # --- Common post-processing for both scoring paths ---
                # Resolved buckets get ranking penalty after thresholding
                if meta.get("resolved", False):
                    normalized *= 0.3

                # Threshold check (precise_match_mode always passes)
    ...
    def _calc_topic_match(self, query: str, bucket: dict,
        ...
        if short_cjk_query:
            if self._is_lexical_stop_term(short_cjk_query):
                return {"score": 0.0, "field_scores": {}, "matched_in": []}
            score = self._calc_short_cjk_topic_score(short_cjk_query, meta, searchable_content)
            return {"score": score, "field_scores": {}, "matched_in": []}

        score = self.calc_topic_scores(query, [bucket]).get(str(bucket.get("id") or ""), 0.0)
        return {"score": score, "field_scores": {}, "matched_in": []}
